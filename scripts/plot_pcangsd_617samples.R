...
DATADIR <- "E:/Dropbox (MERLAB)/Eleni/pollock/results/pcangsd/samples617_maf0.05_miss0.3"
COV_FILE <- "results_pca_samples617_maf0.05_miss0.3.cov"
BAM_LIST <- "samples617_bam.filelist" 
METADATA <- "E:/Dropbox (MERLAB)/Eleni/pollock/sample_metadata/collection_site_metadata.txt"

OUTFILE <- "pcangsd_samples617_maf0.05_miss0.3.pdf"
...
pca_plot <- ggplot(data = plotting_df) +
  geom_point(aes(x = V1 , y = V2, color = interaction(region, pop, sep = " - "), shape = region), 
             alpha = 0.7, size = 2 ) +
